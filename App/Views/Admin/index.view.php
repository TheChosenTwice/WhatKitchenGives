...
<script>
    // Admin config for external script
    window.AdminConfig = {
        deleteUrl: '<?= $link->url("admin.delete") ?>',
        saveUrl: '<?= $link->url("admin.save") ?>'
    };

    // Small UX: hide root navbar in admin
    (function(){ if (typeof document !== 'undefined') { document.addEventListener('DOMContentLoaded', function(){ var nav = document.querySelector('nav.navbar'); if (nav) { nav.style.display = 'none'; } }); } })();
</script>
